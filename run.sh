...
export NVSHMEM_SYMMETRIC_SIZE=$((128000000 * 8 * 20))
...
if [ $RUN_REDUCE_SCATTER -eq 1 ]; then
    NVSHMEM_BOOTSTRAP_UID_SOCK_IFNAME=bond0 bash launch2.sh --nproc_per_node=${NUM_GPUS} reduce_scatter.py
else
    NVSHMEM_BOOTSTRAP_UID_SOCK_IFNAME=bond0 bash launch2.sh --nproc_per_node ${NUM_GPUS} all_gather.py
fi

...
export NVSHMEM_SYMMETRIC_SIZE=${NVSHMEM_SYMMETRIC_SIZE:-15000000000}
...
TORCHRUN=torchrun
if [ ! -z "$PROFILED" ]; then
  TORCHRUN="nsys profile --trace=cuda,nvtx -o $RUN_NAME --force-overwrite true --capture-range=cudaProfilerApi --capture-range-end=stop torchrun"
fi
CMD="${TORCHRUN} \
  --node_rank=${node_rank} \
